        ...
        if(strcmp(cmd,"mkdir")==0){
            ...
            while(1){
                ...
                if(st.st_mode&S_IXOTH)
                    mode[9]='x';
                mode[10]='\0';
                offset += snprintf(temp+offset,len - offset,"%s ",mode);
                offset += snprintf(temp+offset,len - offset,"%d ",1);
                struct passwd *pwd;
                ...
                offset += snprintf(temp+offset ,len - offset,"%s ",pwd->pw_name);
                struct group *grp;
                if((grp = getgrgid(st.st_uid)) == NULL){
                    printf("错误！grp失败\n");
                    closedir(dir);
                    close(fd_accept);
                    printf("◆  当前连接数：%d\n",--pram->count);
                ...
                offset += snprintf(temp+offset,len - offset,"%s ",grp->gr_name);
                offset += snprintf(temp+offset,len - offset,"%d ",(int)st.st_size);
                struct tm *ptm;
                char timebuf[1024];
                int timelen;
                ptm = localtime(&st.st_mtime);
                if(ptm==NULL||(timelen=strftime(timebuf,sizeof(timebuf),"%Y %B %d %H-%M-%S",ptm))==0){
                    printf("错误！ptm失败\n");
                    closedir(dir);
                    close(fd_accept);
                    printf("◆  当前连接数：%d\n",--pram->count);
                ...
                timebuf[timelen]='\0';
                offset += snprintf(temp+offset,len - offset,"%s ",timebuf);
                offset += snprintf(temp+offset,len - offset,"%s\r\n",filename);
            }
            if(write(fd_accept,temp,sizeof(temp))<0){
                closedir(dir);
                printf("错误！打开目录文件失败\n");
                close(fd_accept);
                printf("◆  当前连接数：%d\n",--pram->count);
